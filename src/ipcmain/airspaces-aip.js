...
ipcMain.handle('openaip:download', async (event, args) => {
    const result = await downloadAirspaces(args)
    if (result.success) {   
...
ipcMain.handle('openaip:process', async (event, args) => {
    const result = await processDecoding(args)
    return result
})

async function downloadAirspaces(args) {
    const airfilter = args.values;
    const feature = args.feature;
    console.log('downloadAirspaces request ',airfilter)
